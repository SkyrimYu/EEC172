...
#define POSTHEADER1 "GET /words/"
#define POSTHEADER2 "/definitions HTTP/1.1\r\n"
#define HOSTHEADER "Host: wordsapiv1.p.rapidapi.com\r\n"
#define CHEADER "X-RapidAPI-Key: 08a818e0d1mshf19de409e0f710ep15bef5jsn55331c224a0f\r\n"
#define CTHEADER "Authorization: OAuth\r\n"
...
int ParseJSONData(char *ptr)
{
    ...
    else
    {
        ...
        jsmntok_t json_value = tokenList[2]; // the word searched
        int string_length = json_value.end - json_value.start;

        int idx;

        for (idx = 0; idx < string_length; idx++){
            value[idx] = ptr[json_value.start + idx ];
        }
        ...
        jsmntok_t json_object;
        int i;
            json_object = tokenList[7];

            string_length = json_object.end - json_object.start;


            for (idx = 0; idx < string_length; idx++){
                value[idx] = ptr[json_object.start + idx ];
            }
            ...
            for (i = 0; i < string_length; i++) {
                drawChar(top.x, top.y, value[i], WHITE, BLACK, 1);
                if (top.x + 6 >= 126) {
                    top.y += 8;
                    top.x = 0;
                } else {
...
static int http_post(int iTLSSockID){
    char acSendBuff[512];
     ...
     if(lRetVal < 0) {
         ...
         pcBufHeaders = definition;

         char *token, *last;
         last = token = strtok(acRecvbuff, "\r\n");
         for (;(token = strtok(NULL, "\r\n")) != NULL; last = token);

         strcpy(pcBufHeaders, last);
         pcBufHeaders += strlen(last);
         pcBufHeaders--;
         strcpy(pcBufHeaders, "\0");
         pcBufHeaders++;

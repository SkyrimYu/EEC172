...
//*****************************************************************************
void testHlines() {
    unsigned int y, index = 0;
    unsigned int color[8] = {0x0000, 0x001F ,0x07E0 ,0x07FF, 0xF800,0xF81F,0xFFE0,0xFFFF};

   fillScreen(BLACK);
   for (y=0, index = 0; y < height()-1; y++) {
       if (y % 16 == 0) {
           index++;
       }
     drawFastHLine(0, y, width()-1, color[index]);
   }
     delay(100);
}
...
void testVlines() {
    ...
    unsigned int color[8] = {0x0000, 0x001F ,0x07E0 ,0x07FF, 0xF800,0xF81F,0xFFE0,0xFFFF};
   fillScreen(BLACK);
   for (x=0, index = 0; x < width()-1; x++) {
       if (x % 16 == 0) {
           index++;
       }
     drawFastVLine(x, 0, height()-1, color[index]);
   }
     delay(100);
}
...
void test() {
    while (1) {
        ...
        testlines(CYAN);
        delay(100);
        testfastlines(CYAN,RED);
        delay(100);
        testdrawrects(YELLOW);
        delay(100);
        testfillrects(WHITE, RED);
        delay(100);
        fillScreen(BLACK);
        testfillcircles(5, BLUE);
        delay(100);
        fillScreen(BLACK);
        testroundrects();
        delay(100);
